...
class Ui_Dialog_Tang_Giam_TP(object):
    ...
    def on_slider_moved(self, value):
        ...
        self.label_4.setText(f"Mức độ tương phản {value}%")
        if os.path.isdir(self.global_folder_path_root):
            files = os.listdir(self.global_folder_path_root)
            ...
            if image_files:
                ...
                # Áp dụng tăng/giảm độ tương phản dựa trên giá trị value
                # Tạo một ảnh trống cùng kích thước với img để chứa kết quả
                contrast_img = np.zeros_like(img, img.dtype)

                if value != 0:
                    # Thay đổi độ tương phản, giá trị alpha được xác định bởi giá trị của thanh trượt
                    alpha = 1.0 + value / 100.0  # Hệ số tăng giảm tương phản
                    beta = 0  # Không thay đổi độ sáng

                    # Dùng hàm addWeighted để điều chỉnh độ tương phản của ảnh
                    contrast_img = cv2.addWeighted(img, alpha, contrast_img, 0, beta)
                else:
                    contrast_img = img
    ...
    def handel_locNhieu(self):
        ...
        tienIch.thay_doi_tuong_phan(self.global_folder_path_root, self.global_folder_path_save, giaTriLoc)
        self.progressBar.setValue(100)

    def retranslateUi(self, Dialog):
        _translate = QtCore.QCoreApplication.translate
        Dialog.setWindowTitle(_translate("Dialog", "Dialog"))
        self.label.setText(_translate("Dialog", "<html><head/><body><p>Tăng giảm độ tương phản ảnh</p></body></html>"))
        ...
        self.label_4.setText(_translate("Dialog", "Mức độ tương phản"))
