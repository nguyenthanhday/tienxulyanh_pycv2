...
class Ui_Dialog_tg_Nhieu(object):
    def setupUi(self, Dialog, global_folder_path):
        ...
        self.sl_mucDo.setTickInterval(10)
    ...
    def chooseDirectory(self):
        ...
        if directory:
            self.global_folder_path_save_loc_nhieu = directory
            self.label_2.setText(directory)  
            
    def on_slider_moved(self, value):
        # Kiểm tra xem thư mục và ảnh có tồn tại
        self.value_blr = value
        self.label_4.setText(f"Mức độ giảm nhiễu {value}%")
        if os.path.isdir(self.global_folder_path_root):
            files = os.listdir(self.global_folder_path_root)
            ...
            if image_files:
                ...
                # Áp dụng giảm nhiễu hoặc tăng nhiễu dựa trên giá trị value
                if value < 0:
                    value *= -1  # Chuyển giá trị âm thành dương để tăng nhiễu
                # Tạo giá trị h cho hàm fastNlMeansDenoisingColored
                h = value * 10
                # Giảm nhiễu hình ảnh
    ...
    def handel_locNhieu(self):
        folder_path_root = self.global_folder_path_root
        folder_path_save = self.global_folder_path_save_loc_nhieu
        giaTriLoc = self.value_blr

        # Kiểm tra nếu folder_path_save là None hoặc chuỗi rỗng
        if not folder_path_save:
            # Hiển thị cảnh báo
            QMessageBox.information(None, "Cảnh báo", "Vui lòng chọn thư mục lưu ảnh trước khi tiếp tục!")
            return

        # Tiếp tục xử lý
        tienIch.giam_nhieu_anh(giaTriLoc, folder_path_root, folder_path_save)
    ...
    def retranslateUi(self, Dialog):
        ...
        self.label.setText(_translate("Dialog", "<html><head/><body><p>Giảm nhiễu ảnh</p></body></html>")+self.global_folder_path_root)
        ...
        self.label_4.setText(_translate("Dialog", "Mức độ giảm nhiễu 0%"))
