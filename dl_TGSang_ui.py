...
class Ui_Dialog_TG_Sang(object):
    ...
    def on_slider_moved(self, value):
        ...
        self.label_4.setText(f"Mức độ sáng {value}%")
        if os.path.isdir(self.global_folder_path_root):
            files = os.listdir(self.global_folder_path_root)
            ...
            if image_files:
                ...
                # Áp dụng tăng/giảm độ sáng dựa trên giá trị value
                # Tạo một ảnh trống cùng kích thước với img để chứa kết quả
                bright_img = np.zeros_like(img, img.dtype)

                if value != 0:
                    # Thay đổi độ sáng, giá trị alpha và beta được xác định bởi giá trị của thanh trượt
                    alpha = 1.0  # Hệ số tăng sáng, giữ nguyên
                    beta = value * 2.55  # Giá trị thay đổi độ sáng, từ -255 đến 255
    ...
    def handel_locNhieu(self):
        ...
        tienIch.tang_giam_sang(self.global_folder_path_root, self.global_folder_path_save, giaTriLoc)
    ...
    def retranslateUi(self, Dialog):
        ...
        self.label.setText(_translate("Dialog", "<html><head/><body><p>Tăng giảm độ sáng ảnh</p></body></html>"))
        ...
        self.label_4.setText(_translate("Dialog", "Mức độ tăng sáng"))
